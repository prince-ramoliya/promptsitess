...
import { Copy, Check, Share2, Bookmark, Crown, Sparkles, X } from 'lucide-react';
    ...
    <>
      <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
        <DialogContent className="max-w-3xl w-[95vw] p-0 overflow-hidden border-border/30 bg-card/95 backdrop-blur-2xl rounded-2xl gap-0">
          ...
          {/* Preview area */}
          <div className="relative aspect-[16/10] w-full overflow-hidden rounded-t-2xl bg-muted/20">
            {previewUrl ? (
              isVideo(previewUrl) ? (
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-center gap-3">
              <h2 className="text-xl font-bold text-foreground tracking-tight flex-1">{title}</h2>
              {isPro && (
            ...
            <div className="relative rounded-xl bg-muted/20 border border-border/30 p-4 overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={handleShare}
                className="p-3 rounded-xl bg-muted/30 border border-border/30 text-muted-foreground hover:text-foreground hover:border-primary/30 transition-all"
                title="Share"
              >
