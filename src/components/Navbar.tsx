    ...
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border/50 bg-background/80 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 group">
          ...
          <span className="text-lg font-bold text-foreground">PromptLab</span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          {isAdmin && (
            <Link to="/admin" className="text-sm text-primary hover:text-primary/80 transition-colors">Admin</Link>
          )}
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
          {user ? (
            <>
              ...
              <button onClick={signOut} className="text-sm text-muted-foreground hover:text-foreground transition-colors">Log Out</button>
            </>
          ) : (
            <>
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-border/50 bg-background/95 backdrop-blur-xl p-6 space-y-4">
          ...
          {isAdmin && <Link to="/admin" onClick={() => setMobileOpen(false)} className="block text-sm text-primary">Admin</Link>}
          {user ? (
