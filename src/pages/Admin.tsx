...
import { Gauge, Blocks, Layers, LogOut, Lightbulb, CreditCard, ChartPie, BadgeDollarSign, Users } from 'lucide-react';
          ...
          {navItems.map(item => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center gap-4 px-4 py-3 rounded-xl text-base font-medium transition-all ${active ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'}`}
              >
