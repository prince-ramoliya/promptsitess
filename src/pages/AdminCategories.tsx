...
const AdminCategories = () => {
  ...
  const startEdit = (cat: Category) => {
    setEditing(cat); setName(cat.name); setShowForm(true);
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => { setShowForm(true); setEditing(null); setName(''); }} className="glow-button text-sm !px-5 !py-2 flex items-center gap-2">
        ...
        {categories.map(cat => (
          <div key={cat.id} className="glass-card px-6 py-4 flex items-center justify-between">
            <div>
              ...
              <span className="text-muted-foreground text-xs ml-3">/{cat.slug}</span>
            </div>
